...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:5000/dashboard/${selectedCourse}/${selectedSemester}/sections`
      )
      .then((res) => {
        if (Array.isArray(res.data)) {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        const [enr, eng, att, sub, raw, rawStats, risk, aItems] =
          await Promise.all([
            ...
            axios.get(`${base}/assignItems`).then((res) => res.data),
          ]);
        setData({
          ...
          riskStudents:
            Number(sessionStorage.getItem("atRiskCount")) || risk.riskStudents,
          atRiskStudentsName: risk.atRiskStudentsName,
          aItems: aItems.assignItems || [],
        });
      } catch (err) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      svg
        ...
        .attr("transform", "rotate(-90)")
        .attr("x", -height / 2)
        .attr("y", 20) // ⬅️ increase to push it right if needed (try 20–30)
        .attr("text-anchor", "middle")
        .attr("font-size", "24px")
        .attr("fill", "#333")
        .text("Number of Submissions");

      const filtered = data.submission.filter(
        (d) => !/midterm|final/i.test(d.assignment)
      );
      ...
      const y = d3
        ...
        .domain([
          ...
          d3.max(
            filtered,
            (d) => (+d.onTime || 0) + (+d.late || 0) + (+d.nosub || 0)
          ),
        ])
        .nice()
        .range([height - margin.bottom, margin.top]);
    ...
    if (
      ...
      data.rawScores &&
      Object.keys(data.rawScores).length > 0
    ) {
      const width = 400;
      const height = 180;
  ...
  return (
    <div>
      <h3>
        {selectedCourse
          ? `${selectedCourse} > Dashboard > Course Overview`
          : "Select a Course"}
      </h3>
        ...
        <div className="box-right">
          <select
            ...
            onChange={(e) => {
              ...
              navigate(
                `/course/${selectedCourse}/${e.target.value}/${selectedSemester}/dashboard`
              );
            }}
          >
          ...
          <select
            ...
            onChange={(e) => {
              ...
              navigate(
                `/course/${selectedCourse}/${selectedSection}/${newSemester}/dashboard`
              );
            }}
          >
      ...
      <div className="card">
        <div className="card-header">
          <div className="enrollment">
            <span className="enrollment-icon">
              ...
              >
                <path
                  d="M42.1983 45.9231C42.1983 40.9695 35.7681 36.9538 27.836 36.9538C19.9039 36.9538 13.4737 40.9695 13.4737 45.9231M53.6881 36.955C53.6881 33.277 50.1432 30.1161 45.0707 28.7321M1.98389 36.955C1.98389 33.277 5.52877 30.1161 10.6013 28.7321M45.0707 16.7315C46.8337 15.0892 47.9432 12.7025 47.9432 10.0462C47.9432 5.09258 44.0851 1.07692 39.3258 1.07692C37.1188 1.07692 35.1055 1.94052 33.5809 3.36077M10.6013 16.7315C8.83829 15.0892 7.7288 12.7025 7.7288 10.0462C7.7288 5.09258 11.5869 1.07692 16.3462 1.07692C18.5532 1.07692 20.5665 1.94052 22.0911 3.36077M27.836 27.9846C23.0768 27.9846 19.2186 23.969 19.2186 19.0154C19.2186 14.0618 23.0768 10.0462 27.836 10.0462C32.5952 10.0462 36.4534 14.0618 36.4534 19.0154C36.4534 23.969 32.5952 27.9846 27.836 27.9846Z"
                  stroke="#093CBD"
                  stroke-width="2"
            ...
            <span className="enrollment-number">
              {data.enrollment} Students Enrolled
            </span>
          </div>
        </div>
        ...
        <div className="first-row">
          ...
          <div className="chart-box">
            ...
            <div
              className="submission-legend"
              style={{ display: "flex", gap: "16px", marginTop: "5px" }}
            >
        ...
        <div className="second-row">
          <div className="at-risk-detect-container">
            <div className="student-detect">
              ...
              <div
                className="detect-box risk-student"
                onClick={() => navigate(`/course/${course}/student-list`)}
              >
            ...
            <div className="low-scoring-quiz">
              ...
              <div className="quiz-list">
                {data.aItems && data.aItems.length > 0 && (
                  <div className="quiz-stats">
                    {data.aItems.map((item, index) => (
                      <div key={index} style={{ marginBottom: "1rem" }}>
                        <strong>{item.assess_item_name} Average:</strong>{" "}
                        {item.avg_score} / {item.max_score}
                        <br />
                        <span>
                          Max: {item.highest_score} &nbsp;&nbsp; Min:{" "}
                          {item.lowest_score}
                        </span>
                      </div>
                    ))}
